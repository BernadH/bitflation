...
class InflationCalculator:
  ...
  def inflation(self, **kwargs):
    ...
    # Cummulative inflation calculations (cummulative_inflation() and snap_inflation()) 
    # are rather slow right now. They need a multithreading approach to work 
    # more efficiently or some hardcoded checkpoints. 
    
    def cummulative_inflation():
      self.start_height = 1
    ...
    try: 
      ...
      print(
        "ERROR: Periodicity not recognized. Periodicity must be one of the following:" +
        "\n\tblock: 1 block." +
        "\n\tdaily: 144 blocks." +
        "\n\tweekly: 1008 blocks." +
        "\n\tmonthly: 4320 blocks." +
    ...
    if inflation_type == "cummulative":
      cummulative_inflation()
    elif inflation_type == "price":
      price_inflation()
    elif inflation_type == "harmonized":
      harmonized_inflation()
    elif inflation_type == "csnap":
      csnap_inflation()
    else:
      raise ValueError(
        "ERROR: Inflation type not recognized. Inflation type must be one of the following:" +
        "\n\n\tcummulative: Calculates the cumulative Block over Block inflation from" +
        "Block Height 1 to a given block height." +
        "\n\n\tprice: Calculates the inflation over a set of blocks defined" + 
        "by periodicity (\"block\", \"daily\", \"weekly\", \"monthly\"," +
        "\"yearly\" or \"epochly\")." +
        "\n\n\tharmonized: Calculates the cumulative inflation over a set of blocks." +
        "Blocks are aggregated over a period of time defined by periodicity." +
        "\n\n\tcsnap: Calculate the cumulative Block over Block" +
        "inflation for a set of blocks."
      )
